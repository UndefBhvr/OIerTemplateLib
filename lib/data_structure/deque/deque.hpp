#ifndef _OITL_DATA_STRUCTURE_DEQUE_HPP
#define _OITL_DATA_STRUCTURE_DEQUE_HPP//C++ Header deque.hpp
...
#if defined(__has_include) && __has_include("../../vector/vector.hpp")
#include "../../vector/vector.hpp"
#else
#define _OITL_DATA_STRUCTURE_VECTOR_HPP
namespace oitl
{
...
#define nullptr NULL
	...
	private:
		vector<_Tp> v1,v2;
		size_t v1head,v2head;//v1 used:[v1head,v1.size()),v2 used:[v2head,v2.size())
	private:
		void check();
	public:
		deque():v1head(0),v2head(0){}
		void push_back(const _Tp &_val);
		void pop_back();
		void push_front(const _Tp &_val);
		void pop_front();
		inline size_t size() const{
			return v1.size()-v1head+(v2.size()-v2head);
		}
		inline bool empty() const{
			return size()==0;
		}
	...
	template<typename _Tp>
	void deque<_Tp>::push_back(const _Tp &_val){
		check();
		if(v2head){
			v2[--v2head]=_val;
		} else{
	...
	template<typename _Tp>
	void deque<_Tp>::push_front(const _Tp &_val){
		check();
		if(v1head){
			v1[--v1head]=_val;
		} else{
